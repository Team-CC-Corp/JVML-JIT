...
public class StringBuilder implements CharSequence, Appendable {
	...
	private void flush() {
		if (position > 0) {
			chain = new Cell(new String(buffer, 0, position), chain);
			buffer = null;
			position = 0;
		}
	}
	...
	public StringBuilder insert(int i, char c) {
		return insert(i, new String(new char[] { c }, 0, 1));
	}
